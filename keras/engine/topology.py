...
class Container(Layer):
    ...
    def save_states(self, filepath, overwrite=True):
        '''Dumps all layer states to a HDF5 file.

        The format is similar to that generated by save_weights
        '''
        import h5py
        # If file exists and should not be overwritten:
        if not overwrite and os.path.isfile(filepath):
            ...
            if not proceed:
                return
        f = h5py.File(filepath, 'w')
        self.save_states_to_hdf5_group(f)
        f.flush()
        f.close()
    ...
    def save_states_to_hdf5_group(self, f):
        if hasattr(self, 'flattened_layers'):
            # Support for legacy Sequential/Merge behavior.
            flattened_layers = self.flattened_layers
        else:
        ...
        for layer in flattened_layers:
            ...
            if hasattr(layer, 'states'):
                symbolic_states = layer.states
                state_values = K.batch_get_value(symbolic_states)
                state_names = []
                for i, (s, val) in enumerate(zip(symbolic_states, state_values)):
                    if hasattr(s, 'name') and s.name:
                        name = str(s.name)
                    else:
                        name = 'param_' + str(i)
                    state_names.append(name.encode('utf8'))
                    g.attrs['state_names'] = state_names
                for name, val in zip(state_names, state_values):
                    param_dset = g.create_dataset(name, val.shape,
                                                  dtype=val.dtype)
                    if not val.shape:
                        # scalar
    ...
    def load_states(self, filepath):
        '''Loads all layer states from a HDF5 save file.

        States are loaded into layers
        only if they share the same name.
        '''
        import h5py
        f = h5py.File(filepath, mode='r')
        if 'layer_names' not in f.attrs and 'model_states' in f:
            f = f['model_states']
        self.load_weights_from_hdf5_group(f)
    ...
    def load_states_from_hdf5_group(self, f):
        if hasattr(self, 'flattened_layers'):
            # Support for legacy Sequential/Merge behavior.
            flattened_layers = self.flattened_layers
        else:
        ...
        for name in layer_names:
            g = f[name]
            if 'state_names' in g.attrs:
                state_names = [n.decode('utf8') for n in g.attrs['state_names']]
                if len(state_names):
                    filtered_layer_names.append(name)
        layer_names = filtered_layer_names
        ...
        for k, name in enumerate(layer_names):
            g = f[name]
            state_names = [n.decode('utf8') for n in g.attrs['state_names']]
            state_values = [g[state_name] for state_name in state_names]
            layer = flattened_layers[k]
            symbolic_states = layer.states
            if len(state_values) != len(symbolic_states):
                raise Exception('Layer #' + str(k) +
                                ' (named "' + layer.name +
                                '" in the current model) was found to '
                                'correspond to layer ' + name +
                                ' in the save file. '
                                'However the new layer ' + layer.name +
                                ' expects ' + str(len(symbolic_states)) +
                                ' states, but the saved states have ' +
                                str(len(state_values)) +
                                ' elements.')
            K.batch_set_value(zip(symbolic_states, state_values))
